...
def do_sweep(system, model_name):
    ...
    print(system, model_name, sweep_id)
    ...
    def train(config=None):
        with wandb.init(config=config) as run:
            config = wandb.config
            dataset_config, model_config, train_config = get_config(system_=system, model_name=model_name, z2u=args.z2u)
            dataset_config.n_test_point = n_test_point
            train_config.device = args.device
            set_config(config, dataset_config, model_config, train_config)
            if data.training_dataset is None:
                training_dataset, validation_dataset = load_dataset(dataset_config, train_config, model_config,
                                                                    test_points=None, run=run)
                begin = time.time()
                numerical_runtime, _ = create_simulation_result(
                    dataset_config, train_config, model_config, test_points=dataset_config.test_points[:1],
