...
def get_config(system_, n_iteration=None, duration=None, delay=None):
    if system_ == 's1':
        dataset_config = DatasetConfig(recreate_training_dataset=True, data_generation_strategy='trajectory', delay=3,
                                       duration=16, dt=0.05, n_dataset=500, n_sample_per_dataset=-1, n_plot_sample=20,
                                       ic_lower_bound=-1, ic_upper_bound=1, integral_method='successive adaptive')
        model_config = ModelConfig(model_name='FNO', n_layer=4, fno_n_modes_height=64, fno_hidden_channels=64)
